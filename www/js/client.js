...
function setButtonsTitle() {
  ...
  tippy(videoBtn, {
    content: "Kamerayı Kapat",
    placement: "right-start",
  });
  tippy(screenShareBtn, {
    content: "Ekran Paylaş",
    placement: "right-start",
  });
  tippy(recordStreamBtn, {
    content: "Konuşmayı Kaydet",
    placement: "right-start",
  });
  tippy(fullScreenBtn, {
    content: "Tam Ekran",
  ...
  tippy(msgerCPBtn, {
    content: "Katılımcılar",
  });
  tippy(msgerClean, {
    content: "Mesajları Sil",
  });
  tippy(msgerSaveBtn, {
    content: "Mesajları Kaydet",
  });
  tippy(msgerClose, {
    content: "Chat'i Kapat",
  });
  tippy(msgerEmojiBtn, {
    content: "Emoji",
  });
  ...
  tippy(msgerCloseEmojiBtn, {
    content: "Emojileri Kapat",
  });

  // settings
  tippy(mySettingsCloseBtn, {
    content: "Ayarları Kapat",
  });
  tippy(myPeerNameSetBtn, {
    content: "İsmini Değiştir",
  });

  // whiteboard btns
  tippy(whiteboardCloseBtn, {
    content: "Beyaz Tahtayı Kapat",
    placement: "right-start",
  });
  ...
  tippy(whiteboardCleanBtn, {
    content: "Tahtayı Temizle",
    placement: "right-start",
  });

  // room actions btn
  tippy(muteEveryoneBtn, {
    content: "Herkesi Sustur",
    placement: "top",
  });
  tippy(hideEveryoneBtn, {
    content: "Herkesi Gizle",
    placement: "top",
  });
}
...
function initPeer() {
  ...
  if (!isWebRTCSupported) {
    ...
    userLog("error", "Bu Tarayıcı WebRTC Tarafından Desteklenmiyor!");
    return;
  }

  // peer ready for WebRTC! :)
  console.log("Server'a Bağlanılıyor");
  ...
  function whoAreYou() {
    ...
    Swal.fire({
      ...
      `,
      confirmButtonText: `Görüşmeye Katıl`,
      showClass: {
    ...
    // popup text
    tippy(initAudioBtn, {
      content: "Ses Kapalı Gir",
      placement: "top",
    });
    tippy(initVideoBtn, {
      content: "Kamera Kapalı Gir",
      placement: "top",
    });
  }
  ...
  function welcomeUser() {
    ...
    Swal.fire({
      background: swalBackground,
      position: "center",
      title: "<strong>Hoş Geldiniz " + myPeerName + "</strong>",
      imageAlt: "mirotalk-welcome",
      imageUrl: aboutImg,
      html:
        `
      <br/> 
      <p style="color:white;">Bu Linki Sadece Katılmasını istedikleriniz ile paylaşınız.</p>
      <p style="color:rgb(8, 189, 89);">` +
        myRoomUrl +
        `</p>`,
      confirmButtonText: `URL yi kopyala`,
      showClass: {
        popup: "animate__animated animate__fadeInDown",
      },
      hideClass: {
  ...
  signalingSocket.on("disconnect", function () {
    console.log("Server'dan Ayrıldınız");
  ...
  signalingSocket.on("addPeer", function (config) {
    ...
    peerConnections[peer_id].ontrack = function (event) {
      ...
      if (ontrackCount === 2) {
        ...
        tippy(remoteHandStatusIcon, {
          content: "Kişilerden Biri Elini Kaldırdı",
        });
    ...
    if (useRTCDataChannel) {
      ...
      signalingSocket.on("onMessage", function (config) {
        console.log("Mesaj Alındı", { msg: config.msg });
    ...
    if (config.should_create_offer) {
      console.log("RTC Teklifi yapılıyor", peer_id);
      // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/createOffer
      peerConnections[peer_id]
        .createOffer()
        .then(function (local_description) {
          console.log("Yerel server teklifi yapılıyor", local_description);
          ...
          peerConnections[peer_id]
            ...
            .then(function () {
              ...
              console.log("yerel tanım teklifi tamamlandı!");
            })
            .catch((e) => {
              console.error("[Error] yerel tanım teklifi", e);
              userLog(
                "error",
                "Yerel Tanım teklifi Sağlanamadı " + e.message
              );
            });
        })
        .catch((e) => {
          console.error("[Error] Gönderim teklifi ", e);
        });
    } // end [if offer true]
  }); // end [addPeer]
  ...
  signalingSocket.on("sessionDescription", function (config) {
    console.log("oturum kontrolü", config);
    ...
    peerConnections[peer_id]
      ...
      .then(function () {
        ...
        if (remote_description.type == "offer") {
          console.log("ceap yazılıyor");
          // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/createAnswer
          peerConnections[peer_id]
            .createAnswer()
            .then(function (local_description) {
              console.log("Tanım cevabı: ", local_description);
              // https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/setLocalDescription
              peerConnections[peer_id]
                .setLocalDescription(local_description)
                .then(function () {
                  ...
                  console.log("yerel yanım cevabı tamamlandı!");
                })
                .catch((e) => {
                  console.error("[Error] yerel tanım cevabı", e);
                  userLog(
                    "error",
                    "yerel tanım cevabı sağlanamadı: " + e.message
                  );
                });
            })
            .catch((e) => {
              console.error("[Error] cevap yazma", e);
            });
        } // end [if type offer]
      })
      .catch((e) => {
        console.error("[Error] oturum kontrolü", e);
      });
  ...
  signalingSocket.on("iceCandidate", function (config) {
    ...
    peerConnections[peer_id]
      ...
      .catch((e) => {
        console.error("[Error] aday ekleme", e);
      });
  });
  ...
  signalingSocket.on("removePeer", function (config) {
    console.log("server kaldırmayı istedi:", config);
...
function setupLocalMedia(callback, errorback) {
   ...
   */
  console.log("Ses ve Videona ulaşım isteniyor");
  ...
  navigator.mediaDevices
    ...
    .then(function (stream) {
      console.log("video ve sesine ulaşıldı ");
      ...
      console.log("yerel video ve ses", {
    ...
    .catch((e) => {
      ...
      console.error("video ve ses reddedildi", e);
      playSound("error");
...
function setChatRoomBtn() {
  ...
  msgerCPBtn.addEventListener("click", (e) => {
    if (!thereIsPeerConnections()) {
      userLog("info", "Katılımcı bulunamadı");
      return;
    }
  ...
  msgerSaveBtn.addEventListener("click", (e) => {
    ...
    userLog("info", "Kaydedilebilecek Mesaj Yok");
  });
  ...
  msgerSendBtn.addEventListener("click", (e) => {
    ...
    if (!thereIsPeerConnections()) {
      userLog("info", "Mesaj gösterilecek biri olmadığı için atılamadı");
      msgerInput.value = "";
      return;
    }
...
function attachSinkId(element, sinkId) {
  if (typeof element.sinkId !== "undefined") {
    element
      .setSinkId(sinkId)
      .then(() => {
        console.log(`Ses Çıkışı Atandı: ${sinkId}`);
      })
      .catch((error) => {
        let errorMessage = error;
        if (error.name === "SecurityError") {
          errorMessage = `Ses çıkış cihazı için HPTTS kullanın: ${error}`;
        }
    ...
    console.warn("Tarayıcı seçilen ses çıkışını desteklemiyor.");
  }
}
...
function gotDevices(deviceInfos) {
  ...
  for (let i = 0; i !== deviceInfos.length; ++i) {
    ...
    if (deviceInfo.kind === "audioinput") {
      ...
      // something else
      console.log("Başka tür bir çıkış cihazı: ", deviceInfo);
    }
  } // end for devices
...
function handleError(error) {
  ...
  userLog("error", "Bir şeyler yanlış: " + error.message);
}
...
function attachMediaStream(element, stream) {
  ...
  console.log("Başarılı, medya aktarımı ");
...
async function shareRoomUrl() {
  ...
  if (navigator.share) {
    ...
    try {
      ...
      userLog("info", "Başarı ile paylaşıldı!");
    } catch (error) {
...
function copyRoomURL() {
  ...
  console.log("Link Kopyalandı ", roomURL);
...
function handleAudio(e, init) {
  ...
  if (init) {
    ...
    if (!isMobileDevice) {
      tippy(initAudioBtn, {
        content: myAudioStatus ? "Sesi Kapatmak için Tıklayınız" : "Sesi Açmak için Tıklayınız",
        placement: "top",
      });
    }
  }
...
function handleVideo(e, init) {
  ...
  if (init) {
    ...
    if (!isMobileDevice) {
      tippy(initVideoBtn, {
        content: myVideoStatus ? "Kamerayı Kapatmak İçin Tıklayınız" : "Kamerayı Açmak İçin Tıklayınız",
        placement: "top",
      });
    }
  }
...
function swapCamera() {
  ...
  navigator.mediaDevices
    ...
    .catch((e) => {
      console.log("[Error] kamera değiştirme", e);
      userLog("error", "kamera değiştimede hata: " + e.message);
    });
}
...
function toggleScreenSharing() {
  ...
  screenMediaPromise
    ...
    .catch((e) => {
      console.error("[Error] Ekran Paylaşımı başarısız oldu", e);
      userLog("error", "Ekran paylaşılamaz: " + e.message);
    });
}
...
function setScreenSharingStatus(status) {
  ...
  if (!isMobileDevice) {
    tippy(screenShareBtn, {
      content: status ? "Paylaşımı durdur" : "Paylaşıma başla",
      placement: "right-start",
    });
  }
}
...
function setMyVideoStatusTrue() {
  ...
  if (myVideoStatus === false) {
    ...
    if (!isMobileDevice) {
      tippy(videoBtn, {
        content: "Kamerayı kapatmak için tıkla",
        placement: "right-start",
      });
    }
  }
...
function toggleFullScreen() {
  ...
  if (!isMobileDevice) {
    tippy(fullScreenBtn, {
      content: isDocumentOnFullScreen ? "Tam ekrandan çık" : "Tam ekran",
      placement: "right-start",
    });
  }
}
...
function startStreamRecording() {
  ...
  if (!MediaRecorder.isTypeSupported(options.mimeType)) {
    console.error(`${options.mimeType} desteklenmiyor`);
  ...
  try {
    ...
    console.error("Kayıtta Hata:", e);
    userLog("error", "Kayıt Başlatılamadı: " + e.message);
    return;
  }
  ...
  mediaRecorder.onstop = (event) => {
    ...
    myVideoParagraph.innerHTML = myPeerName + " (ben)";
    disableElements(false);
    downloadRecordedStream();
    // only for desktop
    if (!isMobileDevice) {
      tippy(recordStreamBtn, {
        content: "Kayıda Başla",
        placement: "right-start",
      });
    }
  };
...
function downloadRecordedStream() {
  ...
  userLog(
    "başarılı",
    `kaydedilen dosya ismi: ${recFileName}, başarılı olması için bekleyin, sonra indirilecek ${currentDevice} cihaz.`
  );
...
function addMsgerPrivateBtn(msgerPrivateBtn, msgerPrivateMsgInput, peer_id) {
  ...
  msgerPrivateBtn.addEventListener("click", (e) => {
    ...
    appendMessage(
      ...
      rightChatAvatar,
      "right",
      pMsg + "<br/><hr>Özel mesaj " + toPeerName,
      true
    );
...
function setMyAudioStatus(status) {
  ...
  tippy(myAudioStatusIcon, {
    content: status ? "Sesim Açık" : "Sesim Kapalı",
  });
  // only for desktop
  if (!isMobileDevice) {
    tippy(audioBtn, {
      content: status ? "Sesini kapatmak için tıkla" : "Sesini açmak için tıkla",
      placement: "right-start",
    });
  }
}
...
function setMyVideoStatus(status) {
  ...
  tippy(myVideoStatusIcon, {
    content: status ? "Kameram Açık" : "Kameram Kapalı",
  });
  // only for desktop
  if (!isMobileDevice) {
    tippy(videoBtn, {
      content: status ? "Kamerayı kapatmak için tıkla" : "Kamerayı açmak için tıkla",
      placement: "right-start",
    });
  }
}
...
function setPeerAudioStatus(peer_id, status) {
  ...
  tippy(peerAudioStatus, {
    content: status ? "Katılımcının sesi açık" : "Katılımcının sesi kapalı",
  });
}
...
function setPeerVideoStatus(peer_id, status) {
  ...
  tippy(peerVideoStatus, {
    content: status ? "Katılımcının kamerası açık" : "Katılımcının kamerası kapalı",
  });
}
...
function cleanBoard() {
  ...
  Swal.fire({
    ...
    position: "center",
    title: "Temizle",
    text: "Temizlemek istediğine emin misin?",
    showDenyButton: true,
    confirmButtonText: `Yes`,
    denyButtonText: `No`,
    showClass: {
...
function setMyAudioOff(peer_name) {
  ...
  setMyAudioStatus(myAudioStatus);
  userLog("info", peer_name + " Sesini Kapattı");
}
...
function setMyVideoOff(peer_name) {
  ...
  setMyVideoStatus(myVideoStatus);
  userLog("info", peer_name + " Kameranı Kapattı");
}
...
function disableAllPeers(element) {
  ...
  Swal.fire({
    ...
    text:
      element == "audio"
        ? "susturursan seslerini açamazsın kendileri açabilirler."
        : "saklarsan kameralarını birdaha açamazsın kendileri açabilirler",
    showDenyButton: true,
...
function kickOut(peer_id, peerKickOutBtn) {
  ...
  Swal.fire({
    ...
    imageUrl: confirmImg,
    title: "At " + pName,
    text: "Bu katılımcıyı atmaktan emin misin?",
    showDenyButton: true,
    confirmButtonText: `Yes`,
    denyButtonText: `No`,
    showClass: {
...
function getAbout() {
  ...
  Swal.fire({
    background: swalBackground,
    position: "center",
    title: "<strong>WebRTC İle Yapılmıştır ❤️</strong>",
    imageAlt: "Sanal Kampus",
    imageUrl: aboutImg,
